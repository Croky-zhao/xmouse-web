...
- **`<id>_preview.png`** — 卡片中显示的预览图
  - 建议像素尺寸：**64x64**（@2x 约定，逻辑显示 32x32）
  - 带透明背景
- **`<id>_actual.png`** — 实际应用为鼠标指针的图
  - 建议像素尺寸：**32x32**（@2x 约定，逻辑显示 16x16 像鼠标光标大小）
  - 带透明背景

`<id>` 必须是英文短名（如 `red_arrow`、`heart`），全小写下划线分隔。

> ⚠️ **重要约定**：所有 PNG 默认按 **@2x retina** 处理 — 即上传的像素尺寸是显示尺寸的 2 倍。
> 如果你上传一张 32x32 像素的 actual PNG，鼠标将显示成 **16x16 逻辑点**。
> 如果想要鼠标显示成 32x32 逻辑点，**请上传 64x64 像素的 PNG**。
